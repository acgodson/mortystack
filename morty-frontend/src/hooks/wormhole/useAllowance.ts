...
import { getTokenBridgeAddressForChain } from "@/utils/wormhole/consts";
...
export default function useAllowance(
  ...
  const { isApproving, setIsApproving }: any = useWormholeContext();

  const [allowance, setAllowance] = useState<BigInt | null>(null);
  const [isAllowanceFetching, setIsAllowanceFetching] = useState(false);
  const isApproveProcessing = isApproving;
